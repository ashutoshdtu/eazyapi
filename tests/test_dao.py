...
from eazyapi.dao import SQLDAO, BaseDAO, DatabaseConfig
...
def test_validate_filters_with_invalid_value_data_type() -> None:
    """Tests the validate_filters function with an invalid value data type."""
    filters = {
        "column1": {"$eq": ["value1"]},
    }
    assert BaseDAO.validate_filters(filters) == filters
...
def test_validate_sort_invalid_list_item() -> None:
    ...
    with pytest.raises(InvalidQueryError) as e:
        BaseDAO.validate_sort([("name", 1), "age"])
...
def test_validate_sort_invalid_type() -> None:
    """Tests the validate_sort function with an invalid type."""
    with pytest.raises(InvalidQueryError) as e:
        BaseDAO.validate_sort(123)
    assert (
        str(e.value) == "Invalid sort type: int, allowed types are str, dict, and list of tuples"
    )
